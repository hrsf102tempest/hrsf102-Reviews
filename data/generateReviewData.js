var faker = require('faker');var restaurantData = require('./fecData.js')
...
var review = function(restaurantId) {
  var obj = {
	...
	reviewerId: Math.ceil(Math.random() * Math.floor(25)),
  stars: Math.floor(Math.random() * Math.floor(5)),
  reviewDate: faker.date.recent(),
  checkIns: Math.floor(Math.random() * Math.floor(5)),
  textBody: faker.lorem.paragraphs(Math.ceil(Math.random() * Math.floor(4)))
  };
  return obj;
}
...
var reviews = function() {
  ...
  for (var i = 1; i <= data.length; i++){
	var numberOfReviews = Math.ceil(Math.random() * Math.floor(10));
